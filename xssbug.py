    ...
    def scan_xss(self, url):
        try:
            ...
            for script in soup.find_all('script'):
                if 'document.cookie' in script.text:
                    self.vulnerabilities.append(f'XSS detected at {url}')
            self.crawl(soup)
...
def scan(domain_name):
    ...
    scanner = VulnerabilityScanner(domain_name)
    scanner.scan_xss(domain_name)
